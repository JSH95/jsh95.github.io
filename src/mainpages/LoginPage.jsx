...
function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const { success, message, token, role, username: responseUsername} = response.data;
      if (success) {
        // console.log("로그인 성공:", message);
        login(token, role, responseUsername); // 수정된 변수명 적용
        // navigate(role === "ROLE_ADMIN" ? "/dashboard" : "/workSchedule/list");
        if(role === "ROLE_ADMIN") {
          navigate("/dashboard");
        } else {
  ...
  return (
        <div className="login_container" style={{ backgroundImage: `url(${containerImage})` }}>
